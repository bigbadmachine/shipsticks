...
RSpec.describe ProductsController, type: :controller do
  ...
  let(:valid_attributes) {
    FactoryBot.build(:product).attributes.symbolize_keys
  }

  let(:invalid_attributes) {
    FactoryBot.build(:invalid_product).attributes.symbolize_keys
  }

  let(:updated_attributes) {
    FactoryBot.build(:updated_product).attributes.symbolize_keys
  }
  ...
  describe "PUT #update" do
    context "with valid params" do
      it "updates the requested product" do
        product = Product.create! valid_attributes
        put :update, params: {id: product.to_param, product: updated_attributes}, session: valid_session
        product.reload
        expect(product.length).to eq(27)
        expect(product.width).to eq(27)
        expect(product.height).to eq(27)
        expect(product.weight).to eq(27)
      end
